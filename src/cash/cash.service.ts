...
type PaginatedListFilters = ListFilters & {
  page?: string;
  pageSize?: string;
};
...
export class CashService {
  ...
  private normalizePositiveInt(value: string | undefined, fallback: number): number {
  ...
  private buildWhere(company: string, filters: ListFilters = {}): Prisma.CashEntryWhereInput {
  ...
  async findAll(company: string, filters: ListFilters = {}) {
    const where = this.buildWhere(company, filters);
  ...
  async findPaginated(company: string, filters: PaginatedListFilters = {}) {
    const page = this.normalizePositiveInt(filters.page, 1);
    const rawPageSize = this.normalizePositiveInt(filters.pageSize, 10);
